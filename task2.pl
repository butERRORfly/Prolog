...
average_grade_per_subject_alt :-
    write('-------------------------'), nl,
    write('Средний балл по предметам'), nl,
    write('-------------------------'), nl,
    findall(_, (
        subject(Code, Name),
        ...
        format('~w (~w): ~2f~n', [Name, Code, Average])
    ), _).
...
failed_students_per_subject_alt :-
    write('-------------------------'), nl,
    write('Кол-во не сдавших по предметам'), nl,
    write('-------------------------'), nl,
    findall(_, (
        subject(Code, Name),
        findall(Student, grade(Student, Code, 2), FailedStudents),
        my_length(FailedStudents, Count),
        format('~w (~w): ~w~n', [Name, Code, Count])
    ), _).